...
def model(p, L):
        ...
        xg_reg = xgb.XGBRegressor(objective ='reg:gamma', n_estimators = 100, seed = 42)

        # Train the model
        xg_reg.fit(X_train,y_train)

        # Make predictions
        predictions_xgb = xg_reg.predict(X_test)

        y_pred = predictions_xgb
        #y_pred = scaler_y.inverse_transform(y_pred.reshape(-1,1))

        xgb.plot_importance(xg_reg, importance_type='gain', show_values=False)
        plt.title('Feature Importance based on split gain')
        plt.savefig('xgb.png')
        ...
        plt.savefig('plots_gbm/scatter_p' + str(p) + '_L' + str(L) + '.png')
        ...
        file_path = 'evals_gbm/eval_metrics_p' + str(p) + '_L' + str(L) + '.txt'
