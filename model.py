...
from tensorflow.keras.layers import Conv1D, Conv2D, MaxPooling2D, AveragePooling1D, LeakyReLU
...
def model(p, L):
        ...
        print("test_labels", test_labels_df)
        # TRAINING SET
        ...
        print("training labels:", train_labels_df)
        ...
        #lower_bound = full_df['bindingAffinity'].quantile(0.05)
        #upper_bound = full_df['bindingAffinity'].quantile(0.95)
        ...
        model = Sequential()
        model.add(layers.Dense(128, activation="relu", input_shape=(X.shape[1],),kernel_initializer='he_uniform', kernel_regularizer=regularizers.l2(0.01)))
        ...
        adam = Adam(learning_rate = 0.000075)
        ...
        early_stopping = EarlyStopping(monitor='val_loss', # Monitor the validation loss
                                ...
                                verbose=1,          # To log the number of epoch at which training was stopped
        ...
        history = model.fit(X_train_scaled, y_train_scaled, batch_size = 16, epochs=500, 
                            validation_data = (X_test_scaled, y_test_scaled),verbose=1)#, callbacks=[early_stopping])
                
        plt.figure(figsize=(8, 6))
        plt.plot(history.history['loss'], label='Mean Training Loss')
        plt.plot(history.history['val_loss'], label='Mean Validation Loss')
        plt.xlabel('Epochs')
        plt.ylabel('Loss')
        plt.title('Model loss for p = ' + str(p) + ' and L = ' + str(L))
        plt.legend()
        plt.savefig('plots/loss_p' + str(p) + '_L' + str(L) + '.png')
        ...
        plt.savefig('plots/scatter_p' + str(p) + '_L' + str(L) + '.png')
        ...
        file_path = 'evals/eval_metrics_p' + str(p) + '_L' + str(L) + '.txt'
