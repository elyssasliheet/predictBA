...
def model(p, L):
        ...
        # Predict (don't forget to scale the test set and invert predictions)
        predictions_svr_scaled = svr.predict(X_test_scaled)
        predictions_svr = scaler_y.inverse_transform(predictions_svr_scaled.reshape(-1,1))
        ...
        plt.savefig('plots_svr/scatter_p' + str(p) + '_L' + str(L) + '.png')
        ...
        file_path = 'evals_svr/eval_metrics_p' + str(p) + '_L' + str(L) + '.txt'
